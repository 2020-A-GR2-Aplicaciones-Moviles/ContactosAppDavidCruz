package com.cruzdavid.contactoappcruzdavid.dataBase
...
import com.cruzdavid.contactoappcruzdavid.ContactoModelClass
...
class ContactosSQLiteOpenHelper(context: Context) : SQLiteOpenHelper(context,
    ...
    companion object {
        ...
        const val DATABASE_NAME = "ContactosCruzDavid.db"
    ...
    fun updateContact(contacto : ContactoModelClass) {
        val db = this.writableDatabase
            val values = ContentValues().apply {
                put(COLUMN_NAME_FIRSTNAME, contacto.firstName)
                put(COLUMN_NAME_LASTNAME, contacto.lastName)
